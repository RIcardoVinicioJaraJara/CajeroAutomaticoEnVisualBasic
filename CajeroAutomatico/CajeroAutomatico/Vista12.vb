...
Public Class Vista12
    Private cn As SqlConnection

    Private Sub Vista12_Load(sender As System.Object, e As System.EventArgs) Handles MyBase.Load
    ...
    Public Function retirarDinero(ByVal cantidad As String)
        ...
        Try
            cn.Open()
            Dim salirMovimiento As String = "insert into [dbo].[Movimientos] values(@id,@fecha,'PAGO LUZ',@catidada,@documento,@numcuenta)"
            ...
            cmd1.Parameters.AddWithValue("@catidada", "20")
            cmd1.Parameters.AddWithValue("@documento", "LUZ" + Datos.cli_num_cuenta)
            cmd1.Parameters.AddWithValue("@numcuenta", Datos.cli_num_cuenta)
            Dim cant As Integer = cmd1.ExecuteNonQuery()
            cn.Close()

            cn.Open()
            Dim servicio As String = "insert into [dbo].[Servicios] values(@id,'LUZ',@documento,@numcuenta)"
            Dim cmd2 As SqlCommand = New SqlCommand(servicio, cn)
            cmd2.Parameters.AddWithValue("@id", Datos.max_servicios)
            cmd2.Parameters.AddWithValue("@documento", "LUZ" + Datos.cli_num_cuenta)
            ...
            act1.Parameters.AddWithValue("@cantidad", "20")
